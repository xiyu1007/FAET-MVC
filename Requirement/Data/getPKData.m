function [X,Y] = getPKData()
    % 假设文件名为 'data.data'，第一行是属性名称，第一列是患者ID，最后一列是类别标签（status）
    filename = 'Datasets\parkinsons\parkinsons.txt';
    ...
    % 找到status列的位置
    statusColIndex = find(strcmp(attributeNames, 'status'));  % 查找属性名
    % 提取类别标签（status列）
    Y = data{:, statusColIndex};
    Y = categorical(Y);
    Y = dummyvar(Y);


    featureColumns = setdiff(1:width(data), [1, statusColIndex]);
